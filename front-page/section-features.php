<div class="front_page_section front_page_section_features<?php
	$coala_scheme = coala_get_theme_option( 'front_page_features_scheme' );
	if ( ! empty( $coala_scheme ) && ! coala_is_inherit( $coala_scheme ) ) {
		echo ' scheme_' . esc_attr( $coala_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( coala_get_theme_option( 'front_page_features_paddings' ) );
	if ( coala_get_theme_option( 'front_page_features_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$coala_css      = '';
		$coala_bg_image = coala_get_theme_option( 'front_page_features_bg_image' );
...
<?php
	// Add anchor
	$coala_anchor_icon = coala_get_theme_option( 'front_page_features_anchor_icon' );
	$coala_anchor_text = coala_get_theme_option( 'front_page_features_anchor_text' );
...
?>
	...
	<?php
	if ( coala_get_theme_option( 'front_page_features_fullheight' ) ) {
	...
	?>
			...
			<?php
			$coala_css      = '';
			$coala_bg_mask  = coala_get_theme_option( 'front_page_features_bg_mask' );
			$coala_bg_color_type = coala_get_theme_option( 'front_page_features_bg_color_type' );
			if ( 'custom' == $coala_bg_color_type ) {
				$coala_bg_color = coala_get_theme_option( 'front_page_features_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_features_content_wrap content_wrap">
			<?php
			// Caption
			$coala_caption = coala_get_theme_option( 'front_page_features_caption' );
			if ( ! empty( $coala_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_features_caption front_page_block_<?php echo ! empty( $coala_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $coala_caption, 'coala_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$coala_description = coala_get_theme_option( 'front_page_features_description' );
			if ( ! empty( $coala_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_features_description front_page_block_<?php echo ! empty( $coala_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $coala_description ), 'coala_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_features_output">
				...
				if ( is_active_sidebar( 'front_page_features_widgets' ) ) {
					...
					if ( ! coala_exists_trx_addons() ) {
						...
						coala_customizer_need_widgets_message( 'front_page_features_caption', 'ThemeREX Addons - Services' );
					}
				}
				?>
			</div>
